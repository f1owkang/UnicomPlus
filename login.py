...
#保存cookies以防出现登陆限制@雪狐
def getcookies(username):
    try:
        with open('./session_'+username[-4:]+'.json', 'r', encoding="utf-8") as f:
            data = json.load(f)
            type = True
    except:
        data = ""
...
def savecookies(username, session):
    try:
        d = {}
        d["cookies"] = session.cookies.get_dict()
        d["headers"] = dict(session.headers)
        with open(f'./session_'+username[-4:]+'.json', 'a', encoding="utf-8") as f:
...
def login(username,password,appId):
    ...
    #设置一个标志，用户是否登录成功
    flag = False
    data,type = getcookies(username)
    if type:
        logger.info('【Cookies登录】: ' + username[-4:] + '读取cookies成功！')
        ...
        'User-Agent': 'ChinaUnicom4.x/874 CFNetwork/1240.0.4 Darwin/20.5.0',
        ...
        try:
            ...
            if result['code'] == '0':
                logger.info('【账号登录】: ' + result['default'][-4:])
                session.headers.update({'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 14_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148                       unicom{version:iphone_c@8.0200}{systemVersion:dis}{yw_code:}'})
                flag = True
            else:
                logger.info('【账号登录】: ' + result['dsc'])
        except Exception as e:
            print(traceback.format_exc())
            logger.error('【系统登录】: 发生错误，原因为: ' + str(e))
    if flag:
        data,type = getcookies(username)
        code = savecookies(username, session)
        if not type:
            if code:
                logger.info('【Cookies状态】: 保存 cookies 成功')
            else:
                logger.error('【Cookies状态】: 保存 cookies 失败')
        return session
    else:
        return False
